...
This repository converges one operator's Mac. `./install` is written for this
account, this Homebrew prefix, this home router, this tailnet, this phone, and
this backup host, and it does substantial things to get there: it rewrites
macOS preferences, installs a root-owned helper, and writes sudoers rules. It
is published to be read and forked — take the parts that suit your own machine
— rather than run as a bootstrap. It also requires a sibling `~/code/agentdots`
checkout and stops without one.
...
Nothing on that path opts a cask out of quarantine in the first place. A
`--no-quarantine` in `HOMEBREW_CASK_OPTS` used to sit in `.zshenv`, which
launchd never sources, so neither the scheduled updater nor `./install` ever
read it; removing it changed only interactive `brew install --cask`, which now
quarantines normally and prompts on first launch, as it should. That is
recorded here so the export is not restored later as a fix for a problem it
never solved.
...
The `ssh` row is where `--no-folding` earns its keep. That package ships one
file, `config`, so `~/.ssh` has to stay a real directory holding a single
symlink rather than becoming a symlink into the checkout: `~/.ssh/config.d/` is
...
## Tailnet SSH host files

`ssh/.ssh/config` is tracked and stowed, because it holds `Include` lines and
key handling and nothing about any particular machine. The host aliases it
includes are not tracked at all. A committed host list is a published map of one
person's tailnet — its MagicDNS suffix, every device name on it, and the account
each device is reached as — and the daemon on this machine already knows all of
that and stays right when a device joins, leaves, or is renamed.

`funk ssh-tailnet-config` therefore writes `~/.ssh/config.d/`, one file per
device, from `tailscale status --json`: the MagicDNS suffix, and every device
name including this machine's own, so an alias means the same thing from every
machine on the tailnet. The two things Tailscale cannot report — which account
to log in as, and a non-default port — come from
`~/.config/funk/ssh-tailnet.conf`, which no repository tracks and which is read
in the same `key=value` style as `home-awake.conf`:
...
```
# Generated by funk ssh-tailnet-config; edits are overwritten.
```

and a file under `config.d` whose first line is not exactly that, or that is a
symlink, is never overwritten and never pruned — it is reported instead. That
protects a hand-written host file, and it protects the case that matters: a
leftover Stow link pointing back into this checkout, which a following write
would fill with the data that just left it. The generated files name the
accounts this machine logs in as, so the command holds `config.d` at 0700 and
each file at 0600 itself rather than relying on the install-time `chmod` of
`~/.ssh`, which it can run long after.

`./install` runs the command. A Tailscale that is signed out, stopped, or
unreachable is reported as deferred rather than failing the installation:
nothing is written, nothing is removed, and the aliases already on the machine
go on working. `funk ssh-tailnet-config --check` reports what would change
without writing anything. Nothing regenerates on a schedule — a device that
joins the tailnet after the last run simply has no alias until the command is
run again.
...
This is not a generic bootstrap, as the top of this file says: it assumes this
account, this Homebrew prefix, and this tailnet, and it applies every
machine-specific posture described above. On another machine, fork it and keep
the parts that fit.
...
`home-awake` agents, writes `~/.ssh/config.d` from live Tailscale state, starts
the Yabai/skhd/Karabiner stack, and converges Yabai Spaces 1–9. The
`home-awake` step prompts for `sudo` once on a fresh machine to install its root
helper, then skips the privileged work on every later run.
...
Turning the screen lock off requires the login password that authorizes it, and
storing that password is two steps rather than one. `--set-password` prompts for
it at the terminal — it is never passed as an argument, written to a file, or
logged — and stores it in the login keychain under `funk-home-awake` trusting no
application at all (`-T ""`). Leaving that flag out would have been no safer:
`security(1)` trusts the application that created an item, and the creating
application here is `security` itself, the same binary the agent reads with.
Re-running the step deletes the item before it prompts, so an aborted prompt
leaves nothing stored. `--authorize` then reads the item once, deliberately, at
a terminal, which is what raises the keychain prompt; answer it with Always
Allow, since a plain Allow answers that one read and nothing later.

What that buys is worth stating as plainly as the router address above. Always
Allow puts `/usr/bin/security` back on the item's access list, and from then on
the agent reads the password unattended exactly as it did when storing it handed
out the same grant. This is not secrecy from code running as this user; it is an
explicit human grant in place of one an installer made silently on the
operator's behalf. It is the same argument the root helper above makes about
sudo: trust named by a person for a purpose beats trust handed to a
general-purpose binary anything can invoke.

The unattended path therefore distinguishes three states — the item is missing,
present but unapproved, or approved — and announces each once through the
notifier the rest of home-awake already uses. An unapproved item is held rather
than read again, because this runs every thirty seconds inside the GUI session:
a re-read is another authorization dialog, and one every thirty seconds is not a
failed authorization but an unusable machine. A missing item costs nothing to
retry, since a search that matches nothing never reaches an access list, so that
state re-checks itself and recovers on its own. `funk home-awake --status`
reports the recorded verdict as `screenlock password: <state>` rather than
probing for it, and that verdict lives in
`~/.local/state/funk/home-awake-keychain`, beside the recorded screen-lock
delay. Until the item is stored and approved, home-awake manages sleep only and
leaves the screen lock alone.
...
`silverbird` there is an alias from the generated tailnet host files above, not
a name any resolver answers on its own, so on a machine where
`funk ssh-tailnet-config` has not run there is no route to the repository. The
vault treats an unreachable host as a clean `SKIP silverbird: unreachable`
rather than an error, which is right for a laptop off the tailnet and quiet
enough to hide a missing `~/.ssh/config.d`. Regenerating the host files is a
prerequisite for trusting a run that skipped.
